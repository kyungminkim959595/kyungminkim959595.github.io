...
In addition to the curation of the diagnostic prompts, it is necessary to annotate both the image label and the protected attribute for each generated image. To evaluate the bias of a given text-to-image diffusion model, we first generate a large sample of images for each of the prompt. For example, in the occupation image generation task, we prompt the model as:$``\text{Image of a} ~ \texttt{\{ job \}}''$.
