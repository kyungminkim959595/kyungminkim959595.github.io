...
Text-to-image (T2I) diffusion models have remarkable capabilities in generating realistic images from input text <d-cite key="seshadri2024bias"></d-cite>. However, there are growing concerns about their tendency to produce biased images toward specific demographic groups (e.g. gender, race, age). Because diffusion models learn from large-scale datasets that often encode societal biases, they tend to generate images biased toward specific demographic groups. We define the bias amplification as a phenomenon that a model’s outputs exhibit stronger biases than those present in its training data, as illustrated in Figure 1.
