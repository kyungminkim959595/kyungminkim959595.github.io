...
### Bias Amplification
Bias amplification is the phenomenon where a model's outputs exhibit stronger biases than those present in the its training data <d-cite key="seshadri2024bias"></d-cite>. While such phenomena have been observed in text-to-image diffusion models such as Stabel Diffusion, <d-cite key="seshadri2024bias"></d-cite> empirically showed that in some cases, this may be due to prompt distribution shift-a mismatch between the captions used during training and the prompts provided at the inference time. This finding underscores the importance of careful evaluation on bias amplification.
