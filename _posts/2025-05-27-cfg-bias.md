...
In the context of generative models, group fairness generally refers to ensuring that the model's outputs are demographically balanced across different protected attributes (e.g. gender, race, age). Traditional fairness metrics in supervised learning such as demographic parity is adopted to evaluate the group fairness of generative models. In occupation image generation problem, for example, we say a generative model if group-fair if 

$$
  \mathbb{P}\left( \text{Image of a \{\texttt{job}\}} ~ | ~ \texttt{Subject if female} \right)
  \approx
  \mathbb{P}\left( \text{Image of a \{\texttt{job}\}} ~ | ~ \texttt{Subject if male} \right),
$$
