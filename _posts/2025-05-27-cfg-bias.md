...
In Figure 4, $$p(x ~ | ~ y_1)$$ denotes the **distribution of occupations biased toward females**, $$p(x ~ | ~ y_2)$$ denotes the **distribution of occupations biased toward males**, and $$p(x)$$ denotes the **marginal distribution over all occupations**. The two rightmost plots depict how $$p^w_t(x_t ~ | ~ y)$$ changes for each occupational category. We observe that $$p^w_t(x_t ~ | ~ y)$$ evolves in a manner akin to **extrapolating the conditional distribution from the unconditional distribution**. During the sampling process, this will denoise samples toward $$\nabla \log p^w_t(x_t ~ | ~ y)$$, and will strongly **denoise them toward images of the majority gender** within each occupation.
